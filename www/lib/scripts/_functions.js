...
function renderForm() {
	...
	for(var i in formOptions) {
		...
		if(el.type === 'string') {
			...
			if(el.header !== '' && el.header !== undefined) $('.' + el.classes[0]).append('<option disabled>{text}</option>'.format({text: el.header}));
			if(optionsType === '[object Array]') {
				...
				for(var optionI in el.options) {
					var matSelected = (optionI === config.default_material ? 'selected' : '');
					$('.' + el.classes[0]).append('<option value="{index}" class="{selected}">{text}</option>'.format({
						index: optionI,
